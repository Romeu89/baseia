...
**Step 2 é `blocked_by_missing_infra` (corrigido 2026-04-25 per Finding 4 do REMOTE_REVIEW):** AI execution viable (Haiku gera headlines + A/B + tracking via webhook), validation é mensurável (conversion + webhook + behavioral gate + golden-output A/B). MAS missing_infra não está vazio: feature-flag/experiment infra, analytics webhook, landing CMS com slot dinâmico, opt-in waitlist DB, back_office_optin capture+routing. Por Finding 4 (`ai_executable_at_scale` só com missing_infra vazio), classificação é blocked. Phase 5 unblock: build os 5 itens.

**Step 3 é `blocked_by_missing_infra` (corrigido 2026-04-25 per Finding 4):** AI execution viable (Haiku bot triagem + Sonnet edge cases + Resend determinístico), validation é mensurável (reply rate + webhook + human checkpoint). MAS missing_infra não está vazio: n8n bot infra com confidence-score + scope-check, inbox unificada bot+humano, FAQ KB editável, reply webhook. Phase 5 unblock: build os 4 itens.

**Step 4 é `blocked_by_missing_infra`:** AI execution end-to-end é viável (Sonnet/Haiku faz conciliação, golden-output via rule-based diff é mensurável), MAS infra base não existe. Hoje BaseIA é só FastAPI sem app cliente — Step 4 requer (1) rule-based conciliação engine, (2) Receita Federal seed pipeline, (3) training corpus, (4) app back-office com login/dispute UX, (5) approval state DB. Phase 5 desbloqueio: build esses 5 itens.
...
**Princípio derivado #2 (Finding 4 do REMOTE_REVIEW, corrigido 2026-04-25):** `ai_executable_at_scale` é classificação ASPIRATIONAL apenas quando `missing_infra` está VAZIO. Se missing_infra tem qualquer item, classificação default é `blocked_by_missing_infra` — mesmo com validation_pattern legítimo. Hierarquia: missing_infra > validation_pattern > AI capability. Razão: SDD lockset deve refletir o que **roda hoje**, não o que **rodaria se tivéssemos build**. Aspiration goes na coluna `model_pattern`, gate vai na coluna `classification`. Resultado pós-correção: TODOS os 5 units lockados (phases 2-6) são `blocked_by_missing_infra` — Phase 5 unblock total ~15 itens infra.
